...
const Academy: React.FC<AcademyProps> = ({ gameState, updateState }) => {
  // 筛选条件
  const [selectedSubject, setSelectedSubject] = useState<string>('基础兽医学');
  ...
  const handleStartExam = async () => {
    ...
    try {
      const fetchedQuestions = await getExamQuestions(
        questionCount,
        selectedSubject // 始终传递科目
      );
  ...
  const handleSubmitAnswer = async () => {
    ...
    try {
      const userId = localStorage.getItem('vet-user-id') || 'anonymous-' + Date.now();
      if (!localStorage.getItem('vet-user-id')) {
        localStorage.setItem('vet-user-id', userId);
      }
